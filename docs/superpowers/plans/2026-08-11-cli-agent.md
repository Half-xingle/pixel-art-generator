...
describe('gridToText', () => {
  ...
  it('renders hex grid when color is disabled', () => {
    ...
    assert.ok(out.includes('#00000000'), 'transparent cell shows 8-char hex');
  });
});
```
...
main 的 draw/convert 分支调用处补默认值，并新增 pixels 分支。draw 分支：
...
```js
async function main() {
  const { command, input, size, grid, file, output, palette } = opts;
```
